...
def api_answer():
    ...
    label_msg.config(state="normal")  # Enable editing state
    label_msg.delete("1.0", "end")  # Clear previous content
    label_msg.insert("1.0", ai_response)  # Insert new content
    label_msg.config(state="disabled")  # Disable editing state
    window.update()
...
label_msg = Text(font=("Courier New",15),bg='#0f6b3d',fg="#c2ffe1",relief="flat",state="disabled")
